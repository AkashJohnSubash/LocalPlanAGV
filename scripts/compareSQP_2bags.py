...
sqp_iter1 = np.reshape(mpc1_metric['sqp_iter'][offset_k+1:], (1, iter1-1))
...
sqp_iter2 = np.reshape(mpc2_metric['sqp_iter'][offset_k+1:], (1, iter2-1))
...
sqp_iter1 = np.ravel(sqp_iter1)
sqp_iter2 = np.ravel(sqp_iter2)

# print("\n oscillation 2nd derv:", v1_osc, v2_osc)
fig1 = mpl.pyplot.figure(num='Costs')
costAx = fig1.add_subplot(2, 1, 1)
costAx.stairs(sqp_iter1[:], s1[:], baseline=None,label="DEF", color="lightcoral" )
costAx.stairs(sqp_iter2[:], s2[:], baseline=None,label="LF", color="teal")
costAx.set_ylim( np.hstack([sqp_iter1,sqp_iter2]).max(axis=0) *SCALE_BOUND, 
                 np.hstack([sqp_iter1,sqp_iter2]).min(axis=0) - 2.5)
