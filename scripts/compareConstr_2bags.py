...
avg_stat1 = round(np.abs(stat1).mean(), 4)
avg_ineq1 = round(np.abs(ineq1).mean(), 4)

avg_stat2 = round(np.abs(stat2).mean(), 4)
avg_ineq2 = round(np.abs(ineq2).mean(), 4)

#print(f'Avg. lateral deviation\t: {lat_dev} m')
print(f'Avg. stationarity residual def, lf\t\t: {avg_stat1}, {avg_stat2}')
print(f'Avg. inequality residual \t\t: {avg_ineq1}, {avg_ineq2}')
...
statAx.set_ylim( np.hstack([stat1, stat2]).max(axis=0) , 
                 np.hstack([stat1, stat2]).min(axis=0) )
...
obstBreachAx.set_ylim( np.hstack([np.ravel(slack1_lower[:, :]),
                                  np.ravel(slack2_lower[:, :])]).max(axis=0) + 0.25, 
                       np.hstack([np.ravel(slack1_lower[0, :]),
                                  np.ravel(slack2_lower[0, :])]).min(axis=0) - 0.25 )
...
ineqAx.set_ylim( np.hstack([ineq1, ineq2]).max(axis=0) + 0.01, 
                 np.hstack([ineq1, ineq2]).min(axis=0) - 0.01 )
