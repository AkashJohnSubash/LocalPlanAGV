...
zetaCAx.set_ylim( np.hstack([x1, x2, x2_ref]).max(axis=0) * SCALE_BOUND, 
                  np.hstack([x1, x2, x2_ref]).min(axis=0) -0.4)
...
zetaCAx.set_ylim( np.hstack([y1, y2]).max(axis=0)  + 0.25, 
...
# zetaCAx.set_ylim( np.hstack([phi1, phi2]).max(axis=0)  * SCALE_BOUND, 
...
# UAx.set_xlim(0, 31.5)#np.amax(np.hstack([s1, s2]).max(axis=0)))

# UAx.set_ylabel("$a\,(\\mathrm{m\,s^{-2}})$")
# UAx.invert_yaxis() 
# UAx.legend(loc='upper right', ncol=2)
