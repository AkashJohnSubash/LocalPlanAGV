...
if(lift1 == False):
    ...
    s01 = np.reshape(mpc1_vars['zeta0_3'][offset_k:-1], (1, iter1-1))


if(lift2 == False):
    s02 = np.reshape(mpc2_vars['zeta0_0'][offset_k:], (1, iter2-1))

else:
    s02 = np.reshape(mpc2_vars['zeta0_3'][offset_k:], (1, iter2-1))

t01 = np.reshape(mpc1_metric['soln_time'][offset_k:], (1, iter1))
t02 = np.reshape(mpc2_metric['soln_time'][offset_k:], (1, iter2))
...
fig1 = mpl.pyplot.figure(num='soln_time')
zetaFAx = fig1.add_subplot(2, 1, 1)
zetaFAx.stairs(t1[:-1], s1[:],baseline=None,label="Direct", color="lightcoral" )
zetaFAx.stairs(t2[:-2], s2[:], baseline=None,label="Lifted", color="teal")
zetaFAx.set_xlim( np.hstack([s1, s2]).max(axis=0) * SCALE_BOUND, 
                 np.hstack([s1, s2]).min(axis=0) * SCALE_BOUND)
zetaFAx.set_ylim(0, np.hstack([t1,t2]).max(axis=0))

zetaFAx.set_ylabel("Solution time (s)")
...
costAx.set_xlabel('Track progress $s\,(\\mathrm{m})$')
